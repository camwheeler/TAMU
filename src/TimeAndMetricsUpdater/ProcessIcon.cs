...
namespace TimeAndMetricsUpdater
{
    ...
    {
        private static List<TaskList> tasks;
        private static SpreadsheetsService projectTime;
        ...
        public void Display() {
            ...
            if (File.Exists("user.info"))
                userInfo = JsonConvert.DeserializeObject<UserInfo>(File.ReadAllText("user.info"));
            else{
            ...
            projectTime = new SpreadsheetsService("TimeAndMetrics") {RequestFactory = OAuth2.GetOAuthFactory()};
        }
